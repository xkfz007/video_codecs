 ...
 ************************************************************************
 */
extern void seqSpatialAttention(imgpel**imgY_att,imgpel**imgU_att,imgpel**imgV_att);
...
#if _ATTENTION_
//  if(img->number>0)
	  seqSpatialAttention(pImgOrg[0],pImgOrg[1],pImgOrg[2]);
#endif
...
static void ReportFirstframe(time_t tmp_time, time_t me_time)
{
  ...
  else if (params->Verbose == 2)
  {
	...
	if(params->Distortion[SSIM]==1)
      fprintf (pInfo,"%04d(I)%8d %2d %2d %7.3f %7.3f %7.3f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %6d %5d %3d %2d %2d %d\n",
      frame_no,(int) (stats->bit_ctr - stats->bit_ctr_n),img->AverageFrameQP, lambda, dist->metric[PSNR].value[0], dist->metric[PSNR].value[1],
	  dist->metric[PSNR].value[2], dist->metric[SSIM].value[0], dist->metric[SSIM].value[1], dist->metric[SSIM].value[2],
	  dist->metric[SSIM3D].value[0], dist->metric[SSIM3D].value[1], dist->metric[SSIM3D].value[2],
 dist->metric[STVSSIM].value[0], dist->metric[STVSSIM].value[1], dist->metric[STVSSIM].value[2],
	  (int)tmp_time, (int)me_time,
      intras, img->num_ref_idx_l0_active, img->num_ref_idx_l1_active,img->nal_reference_idc);
  }
...
static void ReportP(time_t tmp_time, time_t me_time)
{
  ...
  else if (params->Verbose == 2)
  {
    ...
    if (params->Distortion[SSIM] == 1)
	 ...
	 if (params->Distortion[SSIM] == 1)
      fprintf (pInfo,"%04d(P)%8d %2d %2d %7.3f %7.3f %7.3f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %6d %5d %3d %2d %2d %d\n",
      frame_no,(int) (stats->bit_ctr - stats->bit_ctr_n),img->AverageFrameQP, lambda, dist->metric[PSNR].value[0], dist->metric[PSNR].value[1],
	  dist->metric[PSNR].value[2], dist->metric[SSIM].value[0], dist->metric[SSIM].value[1], dist->metric[SSIM].value[2],
	  dist->metric[SSIM3D].value[0], dist->metric[SSIM3D].value[1], dist->metric[SSIM3D].value[2],
 dist->metric[STVSSIM].value[0], dist->metric[STVSSIM].value[1], dist->metric[STVSSIM].value[2],
