...
void report( ImageParameters *img, InputParameters *params, StatParameters *stats)
{
  ...
  if(params->Distortion[SSIM]==1)
    fprintf(pAvg,"------------------------------------  Average data all frames  ---------------------------------\n\n");
    break;
  }
  ...
  if (params->Verbose != 0)
  {
	...
	if(params->Distortion[SSIM]==1)
	{
		fprintf(pAvg,  " Total encoding time for the seq.  : %.3f sec (%.2f fps)\n", tot_time*0.001, 1000.0 * (stats->frame_counter) / tot_time);
		fprintf(pAvg,  " Total ME time for sequence        : %.3f sec \n\n", me_tot_time*0.001);

		fprintf(pAvg," Y { PSNR (dB), cSNR (dB), MSE }   : { %5.2f, %5.2f, %5.2f }\n", 
			snr->average[0], csnr_y, sse->average[0]/impix);
		fprintf(pAvg," U { PSNR (dB), cSNR (dB), MSE }   : { %5.2f, %5.2f, %5.2f }\n",
			snr->average[1], csnr_u, sse->average[1]/impix_cr);
		fprintf(pAvg," V { PSNR (dB), cSNR (dB), MSE }   : { %5.2f, %5.2f, %5.2f }\n",
			snr->average[2], csnr_v, sse->average[2]/impix_cr);
	}
    ...
    if (params->Distortion[SSIM] == 1)
    {
      ...
      else
      {
        ...
        fprintf(pAvg," Y SSIM                            : %5.4f\n", dist->metric[SSIM].average[0]);
        fprintf(pAvg," U SSIM                            : %5.4f\n", dist->metric[SSIM].average[1]);
        fprintf(pAvg," V SSIM                            : %5.4f\n", dist->metric[SSIM].average[2]);

        fprintf(pAvg," Y SSIM3D                          : %5.4f\n", dist->metric[SSIM3D].average[0]);
        fprintf(pAvg," U SSIM3D                          : %5.4f\n", dist->metric[SSIM3D].average[1]);
        fprintf(pAvg," V SSIM3D                          : %5.4f\n", dist->metric[SSIM3D].average[2]);
        fprintf(pAvg," Y STVSSIM                         : %5.4f\n", dist->metric[STVSSIM].average[0]);
        fprintf(pAvg," U STVSSIM                         : %5.4f\n", dist->metric[STVSSIM].average[1]);
        fprintf(pAvg," V STVSSIM                         : %5.4f\n", dist->metric[STVSSIM].average[2]);
	fprintf(pAvg,"\n");
    }
...
if(params->Distortion[SSIM]==1)
    fprintf(pAvg, " Total bits                        : %" FORMAT_OFF_T  " (I %" FORMAT_OFF_T  ", P %" FORMAT_OFF_T  ", NVB %d) \n",
      total_bits, stats->bit_counter[I_SLICE], stats->bit_counter[P_SLICE], stats->bit_ctr_parametersets);
  }
  else
  {
  ...
  fprintf(stdout, " Bits for parameter sets           : %d \n\n", stats->bit_ctr_parametersets);
	if(params->Distortion[SSIM]==1)
	{
  fprintf(pAvg, " Bit rate (kbit/s)  @ %2.2f Hz     : %5.2f\n", frame_rate, stats->bitrate / 1000);
  fprintf(pAvg, " Bits to avoid Startcode Emulation : %d \n", stats->bit_ctr_emulationprevention);
  fprintf(pAvg, " Bits for parameter sets           : %d \n\n", stats->bit_ctr_parametersets);
    fprintf(pAvg,"------------------------------------------------------------------------------------------------\n");
  fprintf(pAvg,"Exit JM %s encoder ver %s ", JM, VERSION);
  fprintf(pAvg,"\n");
...
void information_init ( ImageParameters *img, InputParameters *params, StatParameters *stats)
{
  ...
  {
      fprintf(pInfo,"------------------------------------------------------------------------------------------------------------------------------------------------\n");
      fprintf(pInfo,"Frame   Bit/pic QP QL    SnrY    SnrU    SnrV   SsimY   SsimU   SsimV   Sm3dY   Sm3dU   Sm3dV   StsmY   StsmU   StsmV  T(ms) M(ms)   I L0 L1 Ref\n");
      fprintf(pInfo,"------------------------------------------------------------------------------------------------------------------------------------------------\n");
  }
}
