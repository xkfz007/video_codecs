...
void init_enc_mb_params(Macroblock* currMB, RD_PARAMS *enc_mb, int intra, int bslice)
{
  ...
  {
double tmp_lambda;
tmp_lambda=img->lambda_md[img->type][img->masterQP];
#if _ATTENTION_
{
extern  double adjust_lambda(double ,double);
extern  float **att_mbWgt;
int mb_numX=img->width/16;
int mb_y=img->current_mb_nr/mb_numX;
int mb_x=img->current_mb_nr%mb_numX;
double eta;
eta=att_mbWgt[mb_y][mb_x];
    enc_mb->lambda_md =adjust_lambda(tmp_lambda,eta) ;

}
#else
    enc_mb->lambda_md =tmp_lambda;
