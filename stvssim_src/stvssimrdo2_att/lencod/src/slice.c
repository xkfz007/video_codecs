  ...
  while (end_of_slice == FALSE) // loop over macroblocks
  {
    ...
    if (!img->MbaffFrameFlag)
    {
      ...
      else
      {
        ...
        encode_one_macroblock (currMB);//normal
...
void SetLagrangianMultipliersOn()
{
 ...
 double (*lambda_func[])(int)={lambda_poly,lambda_1,lambda_2,lambda_expon,lambda_gauss}; 
          img->lambda_md[j][qp] = lambda_func[params->lambda_func](qp);
#else
