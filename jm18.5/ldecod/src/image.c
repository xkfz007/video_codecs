...
#if OUTPUT_NALU_LEN
    currSlice->slice_len=0;
#endif

    current_header = read_new_slice(currSlice);
#if OUTPUT_NALU_LEN
    //currSlice->slice_len=currSlice->p_Vid->nalu->len;
    //printf("SLICE %d, SIZE %d QP %d\n",currSlice->frame_num,currSlice->p_Vid->nalu->len,currSlice->qp);
    //frm_size+=currSlice->p_Vid->nalu->len;
#endif
    //init;
...
#if OUTPUT_NALU_LEN
      p_Vid->pic_len+=currSlice->slice_len;
#endif
...
process_nalu:
    ...
    case NALU_TYPE_IDR:
        if(nalu->nal_unit_type==NALU_TYPE_IDR)
            printf ("Found NALU_TYPE_IDR, len %d\n", nalu->len);
        else if(nalu->nal_unit_type==NALU_TYPE_SLICE)
            printf ("Found NALU_TYPE_SLICE, len %d\n", nalu->len);
    ...
    case NALU_TYPE_DPB:
      ...
      {
        printf ("found data partition B without matching DP A, len %d discarding\n",nalu->len);
      }
      break;
    case NALU_TYPE_DPC:
      if (p_Inp->silent == FALSE)
      {
        printf ("found data partition C without matching DP A, len %d discarding\n",nalu->len);
      }
      break;
    case NALU_TYPE_SEI:
    ...
    case NALU_TYPE_FILL:
      if (p_Inp->silent == FALSE)
      {
        ...
        //printf ("Skipping these filling bits, proceeding w/ next NALU\n");
          printf ("Found NALU_TYPE_FILL, len %d, skipping\n", nalu->len);
      }
      break;
#if (MVC_EXTENSION_ENABLE)
    case NALU_TYPE_VDRD:
...
#if OUTPUT_NALU_LEN
        fprintf(stdout,"%05d(%s%5d %5d %5d %10d   %s %7d\n",
        p_Vid->frame_no, p_Vid->cslice_type, frame_poc, pic_num, qp,p_Vid->pic_len*8, yuvFormat, (int)tmp_time);
#else
